...
class BotaniqueTaxonAdmin extends Admin
{
    ...
    {
        $accession = $this->container->get('doctrine.orm.entity_manager')->getClassMetadata('NTE\BogartBundle\Entity\BotaniqueTaxon');
...
#        if( (count($session_data) >= 2) && ($session_data[1] !== $user) && ($session_data[0] > $now->format('Y-m-d H:i:s')) ){
        if( $lock ){
            ...
            $formMapper
                ...
                ->end()
                ->with('Synonymes', array('collapsed' => true))
    ...
    #            ->add('croissance', 'choice', array('choices' => array(1 => '<img src="http://localhost/~sanzeyc/bogart/files/symboles/1.png" alt="" />', 
    #                                                                    2 => '<img src="http://localhost/~sanzeyc/bogart/files/symboles/2.png" alt="" />',), 
    #                                                'required'  => false, 
    #                                                'expanded' => true, 
    #                                                'multiple' => true, 
    #                                                )
    ...
    #            ->add('floraison')
                ...
                ->add('remarques')
                ->add('controle')
                ->add('biblio', 'sonata_type_model', array('required' => false, 'empty_value' => '- - - - - -',))
                ->add('semis')
                ->add('melangeTerreNum')
                ->add('melangeTerre')
                ...
                ->add('marchePrintemps', 'sonata_type_boolean')
                ->add('marchePrix')
                ->add('marcheEtiquette')
                ->add('travaux')
                ->end()
                ->with('Informations pour les parcours', array('collapsed' => true))
                ->add('parcoursposition','sonata_type_collection', array('by_reference' => false, 'required' => false, 'label' => 'Parcours / position dans le parcours'), array(
                        'edit' => 'inline',
                        'inline' => 'table',
                    ))
    #            ->add('parcoursposition')
                ...
                ->add('remarqueFr')
                ->add('remarqueDe')
                ->add('fichiers','sonata_type_collection', array('label' => 'Images', 'by_reference' => false, 'required' => false), array(
                        'edit' => 'inline',
                        'inline' => 'table',
                    ))
                ->end()
